...
**EmailПроверка+** — учебный проект для закрепления практических навыков работы с Docker, PHP-FPM, Nginx, Redis Cluster
и фронтендом на Vue.js.

Основная задача — реализовать веб-сервис, принимающий список email-адресов через POST-запрос, проверяющий их
корректность с точки зрения формата и наличия DNS-записи типа MX.
...
**EmailПроверка+** представляет собой **REST API сервис** для валидации email адресов с использованием многоуровневой
архитектуры и Redis кластера для кэширования.
...
- `__construct(SyntaxValidator $syntaxValidator, TldValidator $tldValidator, MxValidator $mxValidator)` — инициализация
  с валидаторами
...
4. **TldValidator** использует **RedisCacheAdapter** (`src/Redis/Adapters/RedisCacheAdapter.php`) для кэширования TLD
   списков
...
- **DomainValidatorInterface** (`src/Interfaces/DomainValidatorInterface.php`) — специализированный интерфейс для
  валидации доменов
