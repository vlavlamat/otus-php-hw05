...
/**
 * Скрипт диагностики подключения к Redis Cluster
 *
 * ВАЖНО: Этот файл должен запускаться ВНУТРИ PHP-контейнера, а не из хост-системы!
 *
 * Запуск внутри контейнера:
 * $ docker exec -it php-fpm1-hw05 php scripts/test_redis_connection.php
 *
 * Функции скрипта:
 * 1. Проверка конфигурации Redis Cluster
 * 2. Тестирование создания Redis адаптера
 * 3. Проверка базовых операций (запись, чтение, проверка существования, удаление)
 * 4. Диагностика доступности всех узлов кластера
 *
 * Успешное выполнение скрипта подтверждает корректную настройку Redis
 * и возможность использования кэширования в приложении.
 */
